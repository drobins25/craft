...
assert_file_contains "flat circle rim width clamp" "clamp(bakeR \* 0.09, 1.3, 3)" "$TEMPLATE"
assert_file_contains "flat circle rim alpha 0.85 and rim boost 55" "bakeR, 0.85, 55)" "$TEMPLATE"
assert_file_contains "vignette inner radius 0.22" "Math.min(w, h) \* 0.22" "$TEMPLATE"
assert_file_contains "vignette outer radius 0.75" "Math.max(w, h) \* 0.75" "$TEMPLATE"
assert_file_contains "vignette fades to 0.4 black" "rgba(0,0,0,0.4)" "$TEMPLATE"
...
assert_file_contains "Sim options match the locked physics" "repulse: 60000, springK: 2.5, springLength: 110, damping: 0.90, gravity: 18, maxSpeed: 6000, alphaDecay: 0.06" "$TEMPLATE"
...
assert_file_contains "seedCompactCloud is called at scale 0.13" "0.13)" "$TEMPLATE"
...
if [ ! -f "$MOCKUP" ]; then
  ...
  PARITY_STRINGS=(
    ...
    "repulse: 60000, springK: 2.5, springLength: 110, damping: 0.90, gravity: 18, maxSpeed: 6000, alphaDecay: 0.06"
    "const RING_R = 380;"
    "2.399963229728653"
