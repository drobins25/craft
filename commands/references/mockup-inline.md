...
**Tool requests (material the alchemist can't fake).** The alchemist's report may carry lines beginning with the exact marker `Tool request:` - naming a typeface (family + weights) or an icon set, and why the faked version would distort this round's reaction. No marker, no flow: skip everything below and proceed as always. When present, the orchestrator fetches the asset files itself - the alchemist never touches the network - and only ever from the pinned sources: Google Fonts for typefaces; the official Lucide, Phosphor, or Heroicons repositories for icons. No other origin is ever fetched from - never hunt the web for font or icon files. Fetched files land in the mockup folder's `assets/` subdirectory, beside mockup.html - inside the mockup's `.craft/` write scope on both warm and cold paths - and the next brief names what arrived and where; the alchemist inlines it (base64 `@font-face` for a font, SVG markup for icons) so the page still makes zero requests at load and still works when the folder moves.
